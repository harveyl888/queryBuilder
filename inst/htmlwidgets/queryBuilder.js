...
//              jsonString += ', "valueGetter": "function(rule) { var value = []; rule.$el.find(\'.rule-value-container input\').each(function() { value.push($(this).selectize()[0].selectize.items) }); return(value) }"';



              jsonString += ", valueGetter: function(rule) { return rule.$el.find(\'.selectized\').selectize()[0].selectize.items; }";
              }

//             $("#input-tags").selectize()[0].selectize.items
...
//        var jsonOperators =  JSON.parse("[" + operator.join() + "]");  // parse all the operators
        var jsonOperators =  "[" + operator.join() + "]";  // parse all the operators
